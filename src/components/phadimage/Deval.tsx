    ...
    <>
      ...
      <Dialog open={showAnimations} onOpenChange={setShowAnimations}>
        <DialogContent className="sm:max-w-lg">
          ...
          <button
            className="mt-4 bg-emerald-500 text-white rounded px-4 py-2 hover:bg-emerald-700"
          >
            Close
          </button>
        </DialogContent>
      ...
      {/* Trigger Button */}
      <motion.button
        className="absolute top-[75%] left-[71%] cursor-pointer z-[50] rounded-full w-4 h-4 bg-emerald-700"
        onClick={handleStartAnimation}
        whileHover={{ scale: 1.2 }}
        whileTap={{ scale: 0.9 }}
      >
        <motion.span
          className="absolute top-0 left-0 w-full h-full rounded-full bg-emerald-400 opacity-50"
